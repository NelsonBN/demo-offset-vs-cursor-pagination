...
public sealed class CursorPageTests
{
    ...
    [Fact]
    public void Get_PaginationCursor_InParts_Should_Return_All_Data()
    {
        // Arrange
        var returnedData = new List<TestPerson>();
        var sort = "id";
        Cursor? cursor = null;
        ...
        // Assert
        returnedData.Should().HaveCount(_testData.Count());
        returnedData.Should().Equal(_testData.OrderBy(p => p.Id));
    }
}
